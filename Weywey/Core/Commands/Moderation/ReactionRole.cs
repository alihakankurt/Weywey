...
public partial class ModerationModule : ModuleBase<SocketCommandContext>
{
    ...
    public async Task ReactionRoleCommand([Summary("The title for reaction roles")] string title, [Remainder][Summary("The reaction role count")] int count)
    {
        ...
        {
            var msg = await InteractionService.WaitForMessageAsync(Context.Channel.Id, TimeSpan.FromSeconds(15), x => x.Author.Id == Context.User.Id && x.MentionedRoles.Count == 1);
            if (msg == null)
            {
            ...
            var reaction = await InteractionService.WaitForReactionAsync(msg.Id, TimeSpan.FromSeconds(15), x => x.UserId == Context.User.Id);
            if (reaction == null)
            {
                await message.DeleteAsync();
                return;
        ...
        ReactionsRoleService.Add(message.Id, dictionary.ToDictionary(x => x.Value.Emote.ToString(), x => x.Key.Id));
    }
}
